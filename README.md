...
- 🎨 **深色反色滤镜** —— 白底画面变柔和深色
- ⏩ **视频倍速控制** —— 纯键盘操作，直接控制 `playbackRate`，突破播放器自带的倍速档位限制（0.125× ～ 16×）
...
不依赖 B 站播放器的固定倍速菜单，而是直接读写 `video.playbackRate`，因此能玩到 **0.125× 慢放** 和 **16× 极速**，还会自动把被播放器重置的速率"钉"回来。
...
> 💬 **倍速悬浮提示**：每次变速时，画面角落会弹出一个**浅灰底、浅白字的小圆圈**显示当前倍速（如 `2x`、`0.125x`），短暂停留后自动淡出。提示会跟随播放器容器，全屏时也能正常显示。
